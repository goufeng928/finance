# GF_PY312_SCRIPT_LLM_IO_Text_to_Vector.py
...
for i in tokenized[     "input_ids"]: formated_dataset[     "input_ids"].append(i)
for i in tokenized["attention_mask"]: formated_dataset["attention_mask"].append(i)
...
print("[Message] Saved as JSON dataset ...")
print("...")

# 将 Python 字典(dict) 导出为 JSON 格式字符串
JSON_DUMPS = json.dumps(formated_dataset, ensure_ascii=False, indent=4)

# 去掉 JSON 格式字符串中第 3 层级的换行和缩进
JSON_DUMPS = JSON_DUMPS.replace("\n            ", str(''))
JSON_DUMPS = JSON_DUMPS.replace("\n        ],", str("],"))
JSON_DUMPS = JSON_DUMPS.replace("\n        ]",   str(']'))

FILE = open("./DATASET_for_LLM_AI_LoRA_IO_Text_Vector.json", mode='w', encoding="utf-8")
